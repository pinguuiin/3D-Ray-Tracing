...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   deprecated_ft_strtod.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ykadosh <ykadosh@student.hive.fi>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/07 21:13:24 by ykadosh           #+#    #+#             */
/*   Updated: 2025/12/16 00:38:18 by ykadosh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int				parse_exponent(char **ptr, size_t n_digits,
							 double *result, size_t line_num);
static inline int		extract_exponent_and_update_result(char **ptr,
						size_t n_digits, double *result);

// 'result', which points at the principal output of this function, has to point
// at a zero value before calling this function.
int	ft_strtod(char **str, double *result, size_t line_num)
{
	...
	if (*ptr == 'e' || *ptr == 'E')
	{
		if (parse_exponent(&ptr, n_digits, result, line_num) == -1)
			return (-1);
		/*
		 * NOTE: delete this when happy with parse_exponent()
		ptr++;
		if (ft_isdigit(*ptr)
...
static int	parse_exponent(char **ptr, size_t n_digits, double *result,
				size_t line_num)
{
	(*ptr)++;
	if (ft_isdigit(**ptr)
